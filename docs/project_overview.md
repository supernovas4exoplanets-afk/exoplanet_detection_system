...
_Critical Constraint: `PC` (Planet Candidate) and `APC` (Ambiguous Planet Candidate) targets are strictly filtered out to prevent label noise during training._
...
- **Visualization & UI:** `Matplotlib` (generating diagnostic plots), `Streamlit` (interactive visual dashboard).
...
```powershell
pip install numpy matplotlib astropy scipy pandas lightkurve astroquery streamlit
```

_(For synthetic generation and modeling, install batman)_:
